...
while True:
    try:
        ...
        def fetch_and_send_links():
            try:
                # XML'den linkleri al
                xml_url = "https://task.haydigiy.com/FaprikaXml/7R1OFT/1/"
        ...
        search_not_in_category_select = Select(driver.find_element("id", "SearchInManufacturerIds"))
        search_not_in_category_select.select_by_value("143")
        
        time.sleep(1)

        third_span = WebDriverWait(driver, 10).until(
            EC.presence_of_all_elements_located((By.CSS_SELECTOR, 'span.select2-selection__choice__remove')))[3]
        third_span.click()
        ...
        category_id_select.select_by_value("143")
